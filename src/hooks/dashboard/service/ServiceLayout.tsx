...
import { createServiceContent, getServiceContents, updateServiceContent, deleteServiceContent } from '@/hooks/dashboard/service/lib/service'

import toast from 'react-hot-toast'

import ServiceSkelaton from "@/hooks/dashboard/service/ServiceSkelaton"
...
export default function HomeLayout() {
    ...
    const [formData, setFormData] = useState<ServiceContent>({
        title: '',
        description: '',
        imageUrl: ''
    })
    ...
    const fetchContents = async () => {
        try {
            setIsLoading(true)
            const data = await getServiceContents()
            setContents(data)
        } catch {
    ...
    const handleDelete = (content: ServiceContent) => {
        setSelectedContent(content)
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (selectedContent) {
                await updateServiceContent(selectedContent.id!, formData, selectedImage || undefined)
                ...
                await createServiceContent(formData, selectedImage)
    ...
    const handleDeleteConfirm = async () => {
        try {
            setIsSubmitting(true)
            await deleteServiceContent(selectedContent!.id!)
            ...
            <div className="flex justify-between items-center py-4 px-6 border-b border-gray-200 bg-primary-50 rounded-md mb-10">
                <div>
                    ...
                    <ul className="flex items-center gap-2">
                        ...
                        <li className="text-sm font-medium">Service</li>
                    </ul>
                </div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {
                    contents.map((content) => (
                        <Card key={content.id} className="max-w-full">
                            <div className="flex flex-col">
                                ...
                                <div className="p-4">
                                    <h3 className="text-2xl font-bold mb-4 text-[var(--primary)]">{content.title}</h3>
