...
import { createAboutContent, getAboutContents, updateAboutContent, deleteAboutContent } from '@/hooks/dashboard/about/lib/about'

import toast from 'react-hot-toast'

import AboutSkelaton from "@/hooks/dashboard/about/AboutSkelaton"
...
export default function HomeLayout() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (selectedContent) {
                await updateAboutContent(selectedContent.id!, formData, selectedImage || undefined)
                ...
                await createAboutContent(formData, selectedImage)
    ...
    const handleDeleteConfirm = async () => {
        try {
            setIsSubmitting(true)
            await deleteAboutContent(selectedContent!.id!)
                        ...
                        <div className='flex flex-col md:flex-row items-center'>
                            ...
                            <div className="p-8 md:w-1/2">
                                <div className='flex flex-col gap-4'>
                                    <h3 className="text-2xl xl:text-4xl max-w-[500px] font-bold text-[var(--primary)]">{content.title}</h3>
                                    <span className='text-2xl font-bold text-[var(--primary)]'>{content.text}</span>
                                    <p className="text-gray-600 mb-8 text-lg leading-relaxed max-w-[450px]">{content.description}</p>
                                </div>
